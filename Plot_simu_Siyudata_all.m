...
%     [res_mean, res_std] = extract_data_foronepercentage_avetime(N_vec, dsmall,dmid,dlarge);
    ...
    [res_mean, res_std] = extract_data_foronepercentage_maxtime(N_vec, dsmall,dmid,dlarge);
    maxtime_mean = [maxtime_mean,res_mean];
    maxtime_std = [maxtime_std,res_std];
...
% if we want to plot the ave time, change
% extract_data_foronepercentage_avetimeratio to extract_data_foronepercentage_avetime
...
% plot_max_time(N_vec,maxtime_mean,maxtime_std)
...
function plot_norm(N_vec,norm_mean,norm_std)
    ...
    for i = 1:size(norm_mean,2)
        if mod(i,2)==1
            errorbar(N_vec, norm_mean(:,i), norm_std(:,i), 's-', 'Color', colors(count), 'LineWidth', 3, 'MarkerSize', 16,'CapSize',12);
        else
            errorbar(N_vec, norm_mean(:,i), norm_std(:,i), '^--', 'Color', colors(count), 'LineWidth', 3, 'MarkerSize', 16,'CapSize',12);
            count = count+1;
        end
    end
...
function [res_mean, res_std] = extract_data_foronepercentage_maxtime(N_vec, dsmall,dmid,dlarge)
    data_mean = zeros(length(N_vec),6);
%     data_std = zeros(length(N_vec),6);
    count = 1;
    for N = N_vec
...
function plot_max_time(N_vec,norm_mean,norm_std)
    fig = figure; hold on;
    % colors = ["#D08082", "#C89FBF", "#62ABC7", "#7A7DB1", "#6FB494", "#D9B382"];
    colors = [ "#C89FBF", "#7A7DB1", "#D9B382", "#6FB494"];
    x = 1:length(N_vec);
    count = 1;
%     norm_mean(2,3) = norm_mean(2,3)+0.25;
...
%     norm_mean(7:11,1) = norm_mean(7:11,1)+ 0.01;
    ...
    for i = 1:size(norm_mean,2)
        if mod(i,2)==1
            plot(N_vec, norm_mean(:,i), 's-', 'Color', colors(count), 'LineWidth', 3, 'MarkerSize', 16);
        else
            plot(N_vec, norm_mean(:,i), '^--', 'Color', colors(count), 'LineWidth', 3, 'MarkerSize', 16);
            count = count+1;
        end
    end
...
function plot_time(N_vec, time_mean, time_std)
    ...
    for i = 1:size(time_mean,2)
        if mod(i,2)==1
            errorbar(N_vec, time_mean(:,i), time_std(:,i), 's-', 'Color', colors(count), 'LineWidth', 4, 'MarkerSize', 12,'CapSize',10);
        else
            errorbar(N_vec, time_mean(:,i), time_std(:,i), '^--', 'Color', colors(count), 'LineWidth', 4, 'MarkerSize', 12,'CapSize',10);
            count = count+1;
        end
    end
...
function plot_scheduled_instances(N, time_window, sheduled_instances_res)
    ...
    for i = 1:size(sheduled_instances_res,2)
        if mod(i,2)==1
            plot(time_window, sheduled_instances_res(:,i), 's-', 'Color', colors(count), 'LineWidth', 3.5, 'MarkerSize', 12);
        else
            plot(time_window, sheduled_instances_res(:,i), '^--', 'Color', colors(count), 'LineWidth', 3.5, 'MarkerSize', 12);
            count = count+1;
        end
    end
