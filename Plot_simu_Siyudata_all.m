...
for i = 1:size(param_sets, 1)
    ...
    [time_window, res] = extract_data_foronepercentage_sheduled_instances(100, dsmall,dmid,dlarge);
...
plot_scheduled_instances(100, time_window, sheduled_instances_res)
...
function plot_norm(N_vec,norm_mean,norm_std)
    ...
    for i = 1:size(norm_mean,2)
        if mod(i,2)==1
            errorbar(x, norm_mean(:,i), norm_std(:,i), 's-', 'Color', colors(count), 'LineWidth', 4, 'MarkerSize', 10,'CapSize',10);
        else
            errorbar(x, norm_mean(:,i), norm_std(:,i), 'o--', 'Color', colors(count), 'LineWidth', 4, 'MarkerSize', 10,'CapSize',10);
            count = count+1;
        end
    end
...
function plot_time(N_vec, time_mean, time_std)
    ...
    for i = 1:size(time_mean,2)
        if mod(i,2)==1
            errorbar(x, time_mean(:,i), time_std(:,i), 's-', 'Color', colors(count), 'LineWidth', 4, 'MarkerSize', 10,'CapSize',10);
        else
            errorbar(x, time_mean(:,i), time_std(:,i), 'o--', 'Color', colors(count), 'LineWidth', 4, 'MarkerSize', 10,'CapSize',10);
            count = count+1;
        end
    end
    ...
    ylabel('$t$(sec)','interpreter','latex',FontSize=24)
