...
% 1. norm:
%--------------------------------------------------------------------------
results_norm = results(:,1:7);
mean_norm = mean(results_norm)
std_norm = std(results_norm)
...
results_avetime = results(:,8:14);
mean_time = mean(results_avetime)
std_time = std(results_avetime)
...
% 3. MAX time:
%--------------------------------------------------------------------------
max_time = max(results_avetime)

A = [mean_norm;mean_time;max_time]
...
%--------------------------------------------------------------------------
plot_scheduled_instance_realworld(results_avetime)



function plot_scheduled_instance_realworld(results)
    
%     time_window = logspace(log10(0.01), log10(0.1), 50);
    time_window = linspace(0.01,0.09,50);
    
    count = 1;
    data_shecduled_instace = zeros(length(time_window),7);
    ...
    for i = [1,3]
        if i==1
            plot(time_window, data_shecduled_instace(:,i)./size(results,1), 's-',  'LineWidth', 3.5, 'MarkerSize', 10);
        else
            plot(time_window, data_shecduled_instace(:,i)./size(results,1), 'o--',  'LineWidth', 3.5, 'MarkerSize', 10);
        end
    end
    
    
    output_matrix = [time_window.', data_shecduled_instace(:,1)./size(results,1),data_shecduled_instace(:,3)./size(results,1)
                 ];   % 如果第7条是重复第一条

    writematrix(output_matrix, 'D:\\data\\ISPP_givenA\\final_data\\scheduled_instance_maxistf_matrix_realworld.csv');
